...
class operator:
  ...
  def conj(self):
    ...
    for term in self.opterms:
      conjTerm = term.conj()
      newOperator = newOperator + operator([opterm(conjTerm.coeff, conjTerm.string)])
  ...
  def expand(self, term):
    ...
    for i in range(len(term.string)):
      if string[i] == "5":
          ...
          newOperator = newOperator + self.expand(opterm( newCoeff1*term.coeff, newString1 ))
          newOperator = newOperator + self.expand(opterm( newCoeff2*term.coeff, newString2 ))
  ...
  def __mul__(self, other):
    ...
    if( type(other) in (int, float, complex) ):
      for term1 in self.opterms:
        newOperator = newOperator + operator([opterm(term1.coeff*other, term1.string)])
      ...
      for term1 in self.opterms:
        ...
        for term2 in other.opterms:
          ...
          if not dead:
            ...
            multiplied = self.expand(multiplied) # Could be a list or a term
  ...
  def __rmul__(self, other):
    ...
    if( type(other) in (int, float, complex) ):
      ...
      return other.__mul__(self) # Always perform multiplication with left term first
